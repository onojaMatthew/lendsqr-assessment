...
export const transfer_fund = async (req, res) => {
  ...
  try {
    ...
    console.log(from[0] && from[0].balance, sender_balance, amount);
...
export const withdraw_fund = async (req, res) => {
  const { amount, debit_acct_no, receiver_name, narrative, sender_name  } = req.body;
  try {
    const to = await knex.select().from("users").where("account_number", debit_acct_no);
    const receiver_balance = to[0] && to[0].balance - amount;

    // console.log(to[0].balance, receiver_balance, amount) 
    // return;
    
    await knex("users").where("account_number", debit_acct_no).update({ balance: receiver_balance });

    await knex("transactions").insert({ 
      from: debit_acct_no, sender_name, amount, transaction_type: "debit", receiver_name, created_by: to[0].id, narrative 
    });
